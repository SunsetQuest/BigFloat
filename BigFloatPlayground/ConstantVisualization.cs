...
public static class ConstantVisualization
{
    ...
    public static string GetContinuedFraction(BigFloat value, int terms = 10)
    {
        ...
        for (int i = 1; i < terms && !remaining.IsZero; i++)
        {
            ...
            remaining = BigFloat.Inverse(remaining);
    ...
    {
        if (!BigFloat.Catalog.TryGetInfo(constantId, out BigFloat.ConstantInfo info))
        {
        ...
        _ = result.AppendLine($"\nPrecision Available:");
        _ = result.AppendLine($"- In Memory: {Math.Round(info.BitsInBase64.Length * 6 / 3.32192809488736235, 0)} decimal digits");
